    ...
    async function fetchCircles() {
        ...
        try {
            ...
            const res = await fetch(
                buildApiUrl(`/api/v1/admin/circles?${params}`),
...
<div class="pb-shell page">
    ...
    <main class="main">
        ...
        {:else if circles.length === 0}
            <div class="empty">등록된 동호회가 없습니다.</div>
        {:else}
            <div class="table-wrap">
                <table class="partner-table">
                    ...
                    <tbody>
                        {#each circles as c, idx}
                            <tr
                                class="clickable"
                                on:click={() => goDetail(c.id)}
                            >
                                <td>{(page - 1) * size + idx + 1}</td>
                                <td class="name-cell">{c.businessPartner}</td>
                                <td>{c.owner}</td>
                                <td>{c.phoneNumber}</td>
                                <td class="addr-cell">{c.partnerAddress}</td>
                                <td>
                                    <span
